...
class ftmAttack(nn.Module):
    def __init__(self,source_models, p,alpha,ftsetting,\
                 targeted=True,attack_type='RTMF',num_iter=300,max_epsilon=16,mu=1.0,returnGrad=True,*args, **kwargs):
    ...
    def forward(self,x,y,target_label):
        assert self.targeted == True, "Not supported for non-targeted attack"
        ...
        # Set loss function
        loss_fn = LogitLoss(target_label, self.targeted)
        ...
        if 'F' in self.attack_type:  # Storing clean features at the first iteration
            with torch.no_grad():
                ...
                models = []
                for source_model in self.source_models:
                    models.append(FeatureTuning(source_model, img_width, self.ftsetting))
                for model in models:
                ...
                assert consumed_iteration < self.num_iter, "consumed_iteration should be less than num_iter"
        else:
            models = self.source_models

        # perform iterative attack
        for t in range(consumed_iteration, self.num_iter):
            x_nes = x_adv.detach()
            x_nes.requires_grad = True

            if 'D' in self.attack_type:
                x_adv_or_nes = DI(x_nes, self.prob)
            ...
            if 'M' in self.attack_type:
                g = self.mu * g + grad_x / torch.sum(torch.abs(grad_x), dim=[1, 2, 3], keepdim=True)
            else:
                g = grad_x
        ...
        if self.returnGrad:
            return x_adv.detach()-x.detach()
        return x_adv.detach()


class FeatureTuning(nn.Module):
    def __init__(self, model: nn.Module, input_size, exp_settings):
        super().__init__()
                ...
                if self.mixup_layer == 'conv_linear_no_last' and (layer_idx + 1) == self.layer_num and type(module) == torch.nn.Linear:
                    ...
                    if layer_idx in self.outputs and self.record == False:  # Feature mixup inference mode
                        ...
                        if self.mixing_triggered[layer_idx]:
                            ...
                            if self.channelwise == True:
                                if output.dim() == 4:
                                    a = (torch.rand(prev_feature.shape[0],
                                        ...
                                        prev_feature.shape[0], prev_feature.shape[1], 1, 1).cuda()
                                elif output.dim() == 3:
                                    a = (torch.rand(prev_feature.shape[0],
                                                    prev_feature.shape[1]) * mix_ratio + mix_lower_bound_feature).view(
                                        prev_feature.shape[0], prev_feature.shape[1], 1).cuda()
                                else:
                                    a = (torch.rand(prev_feature.shape[0],
                                                    prev_feature.shape[1]) * mix_ratio + mix_lower_bound_feature).view(
                                        prev_feature.shape[0], prev_feature.shape[1]).cuda()
                            else:
                                if output.dim() == 4:
                                    a = (torch.rand(prev_feature.shape[0]) * mix_ratio + mix_lower_bound_feature).view(
                                        prev_feature.shape[0], 1, 1, 1).cuda()
                                elif output.dim() == 3:
                                    a = (torch.rand(prev_feature.shape[0]) * mix_ratio + mix_lower_bound_feature).view(
                                        prev_feature.shape[0], 1, 1).cuda()
                                else:
                                    a = (torch.rand(prev_feature.shape[0]) * mix_ratio + mix_lower_bound_feature).view(
                                        prev_feature.shape[0], 1).cuda()
                            ...
                            output_flat = output.detach().flatten(start_dim=1)  # [B, *]
                            tuning_flat = self.outputs_tuning[layer_idx].detach().flatten(start_dim=1)   # [B, *]
                        ...
                        # If not selected, mix the output with feature perturbations
                            ...
                            #output_flat = output.detach().view(output.size(0), -1)  # [B, *]
                            output_flat = output.detach().flatten(start_dim=1) 
                            #tuning_flat = self.outputs_tuning[layer_idx].detach().view(output.size(0), -1)  # [B, *]
                            tuning_flat = self.outputs_tuning[layer_idx].detach().flatten(start_dim=1) 
                    ...
                    elif self.record == True:  # Feature recording modes
